...
2'd1: begin
    ...
    data_out[i*(PNUM*DW)+2*DW+:DW]<=0;   
    data_out[i*(PNUM*DW)+1*DW+:DW]<=0;   
    data_out[i*(PNUM*DW)+0*DW+:DW]<=data_in[i*(PNUM*DW)+0*DW+:DW];
    data_out_vld[i*PNUM+3+:1]<=0;
    data_out_vld[i*PNUM+2+:1]<=0;
    data_out_vld[i*PNUM+1+:1]<=0;
    data_out_vld[i*PNUM+0+:1]<=1;  
end
2'd2: begin 
    data_out[i*(PNUM*DW)+3*DW+:DW]<=0;             
    data_out[i*(PNUM*DW)+2*DW+:DW]<=0;   
    data_out[i*(PNUM*DW)+1*DW+:DW]<=data_in[i*(PNUM*DW)+0*DW+:DW];   
    data_out[i*(PNUM*DW)+0*DW+:DW]<=data_in[i*(PNUM*DW)+1*DW+:DW];
    data_out_vld[i*PNUM+3+:1]<=0;
    data_out_vld[i*PNUM+2+:1]<=0;
    data_out_vld[i*PNUM+1+:1]<=1;
    data_out_vld[i*PNUM+0+:1]<=1;   
end
2'd3: begin 
    data_out[i*(PNUM*DW)+3*DW+:DW]<=0;             
    data_out[i*(PNUM*DW)+2*DW+:DW]<=data_in[i*(PNUM*DW)+0*DW+:DW];   
    data_out[i*(PNUM*DW)+1*DW+:DW]<=data_in[i*(PNUM*DW)+1*DW+:DW];   
    data_out[i*(PNUM*DW)+0*DW+:DW]<=data_in[i*(PNUM*DW)+2*DW+:DW];
    data_out_vld[i*PNUM+3+:1]<=0;
    data_out_vld[i*PNUM+2+:1]<=1;
    data_out_vld[i*PNUM+1+:1]<=1;
    data_out_vld[i*PNUM+0+:1]<=1;   
end
default: begin 
    data_out[i*(PNUM*DW)+3*DW+:DW]<=data_in[i*(PNUM*DW)+0*DW+:DW];             
    data_out[i*(PNUM*DW)+2*DW+:DW]<=data_in[i*(PNUM*DW)+1*DW+:DW];   
    data_out[i*(PNUM*DW)+1*DW+:DW]<=data_in[i*(PNUM*DW)+2*DW+:DW];   
    data_out[i*(PNUM*DW)+0*DW+:DW]<=data_in[i*(PNUM*DW)+3*DW+:DW];
    data_out_vld[i*PNUM+3+:1]<=data_in_vld[i*PNUM+0+:1];
    data_out_vld[i*PNUM+2+:1]<=data_in_vld[i*PNUM+1+:1];
    data_out_vld[i*PNUM+1+:1]<=data_in_vld[i*PNUM+2+:1];
    data_out_vld[i*PNUM+0+:1]<=data_in_vld[i*PNUM+3+:1];  
end
endcase
